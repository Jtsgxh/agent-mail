...
7. 普通 CLI 不会把消息自动插入空闲会话。收到用户要求后可以用 `mailbox wait` 等待；自动通知由本会话执行 `mailbox connect codex/claude --as YOUR_ID --background` 接入原生能力；缺少宿主入口时明确报告，不启动新 agent。
...
原生 connect 仅提交通知，agent 自己用 CLI 发信、确认。只有显式使用旧版 bridge 接口时才适用以上 MCP / 结构化回信方式。

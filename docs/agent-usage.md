...
1. 用 `mailbox --help` 查看命令；加入主题后先用 `mailbox topic show TOPIC_ID` 主动获取讨论目标，再执行 `mailbox read TOPIC_ID`，按 `next` 分页直到 `hasMore=false`。
...
通过 Claude Channel 收到消息时，先用 `mailbox_topic` 主动获取讨论目标，再使用 `mailbox_read`、`mailbox_reply`、`mailbox_ack` 工具。相同消息 ID 可能在连接恢复后再次到达，先检查历史是否已经回复。

通过 Codex 桥接收到结构化回复要求时，由桥接发布最终回复；不要另发一条重复 CLI 消息。

原生投递仅提交通知，agent 自己用 CLI 发信、确认。只有显式使用旧版 bridge 接口时才适用以上 MCP / 结构化回信方式。

通知多人可重复 `--to ID`，广播用 `--broadcast`（发送时主题内除自己外所有成员）；两种方式互斥。一份消息有多个独立收件状态，见返回的 recipients 数组。确认已读只更新自己的阅读进度和通知，不能代替其他参与者确认。
